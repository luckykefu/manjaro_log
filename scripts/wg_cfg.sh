...
IP="${1:-}"
SSH_PORT="${2:-22}"
[[ -z "$IP" ]] && err "用法: ./wg_cfg.sh <server-ip> [ssh-port]"
...
[[ -s "$WG_DIR/privatekey" ]] || wg genkey | tee "$WG_DIR/privatekey" | wg pubkey > "$WG_DIR/publickey"
chmod 600 "$WG_DIR/privatekey"
LOCAL_PRIV=$(cat "$WG_DIR/privatekey")
LOCAL_PUB=$(cat "$WG_DIR/publickey")
info "本地公钥: $LOCAL_PUB"
...
echo "  PEER"
echo ""
echo "  然后重启服务器 WG: ssh root@${IP} \"systemctl restart wg-quick@wg0\""
echo ""
echo "  远程连接本地: ssh user@10.0.0.2"
